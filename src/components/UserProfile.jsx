// src/components/UserProfile.jsx - FIXED
...
const UserProfile = ({ contractAddress, userAddress }) => {
  ...
  // ✅ FIX: Wrapped in useCallback to satisfy dependency requirements
  ...
  }, [userAddress, contractAddress]); // ✅ Added all dependencies

  useEffect(() => {
    loadUserData();
  }, [loadUserData]); // ✅ Now includes loadUserData properly
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 transition-colors">
      ...
      <div className="mb-6">
        <div className="flex justify-between mb-2">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Reputation</span>
          <span className="text-sm text-gray-600 dark:text-gray-400">{reputation} / 10,000</span>
        </div>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{reputationPercent}% of maximum</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-indigo-50 dark:bg-indigo-900 rounded-lg p-4">
          <p className="text-sm text-indigo-600 dark:text-indigo-300 font-medium">Votes Cast</p>
          <p className="text-2xl font-bold text-indigo-700 dark:text-indigo-400">{voteCount}</p>
        </div>
        
        <div className="bg-purple-50 dark:bg-purple-900 rounded-lg p-4">
          <p className="text-sm text-purple-600 dark:text-purple-300 font-medium">Reputation</p>
          <p className="text-2xl font-bold text-purple-700 dark:text-purple-400">{reputation}</p>
        </div>
      </div>

      {/* Delegation Section */}
      <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Voting Delegation</h3>
        ...
        {delegate ? (
          <div className="bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg p-4 mb-4">
            ...
            <p className="font-mono text-sm text-gray-800 dark:text-gray-300 mb-3 break-all">{delegate}</p>
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-yellow-600 dark:bg-yellow-700 text-white py-2 px-4 rounded-lg font-medium hover:bg-yellow-700 dark:hover:bg-yellow-800 disabled:opacity-50 transition-colors"
            >
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full bg-indigo-600 dark:bg-indigo-500 text-white py-2 px-4 rounded-lg font-medium hover:bg-indigo-700 dark:hover:bg-indigo-600 disabled:opacity-50 transition-colors"
            >
      ...
      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg">
        <h4 className="text-sm font-semibold text-blue-900 dark:text-blue-300 mb-2">How to Earn Reputation</h4>
